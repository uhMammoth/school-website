...
const Dropdown = ({ color }) => {
  ...
  return (
    <>
          <img src={Bottom} alt='' className='sticky top-0 w-full h-40' /> 
      <div className=" pt-2 w-full flex flex-wrap">
        <div className=" w-full  px-4">
          <div className=" relative inline-flex align-middle w-full">
            <button
              className={
                "yellow w-full font-bold uppercase text-sm px-6 py-3 rounded-xl shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 " +
                bgColor
              }
              type="button"
              ref={btnDropdownRef}
            ...
            >
              {color === "yellow" ? "Choose a Counselor" : "Choose a Counselor"}
            </button>
            <div
              ref={popoverDropdownRef}
              className= {
                (dropdownPopoverShow ? "block " : "hidden ") +
                (color === "white" ? "covid " : bgColor + " ") +
                "nav text-base z-50 float-left py-2 list-none text-left rounded shadow-2xl mt-1"
      ...
      <img src={Grad} alt='' className='sticky bottom-0 w-full h-40' />
            </div>
